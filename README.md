...
## Скрапінг (опційно)
```bash
# Використайте готовий профіль для onlyart.org.ua з пагінацією ?paged=N.
...
Якщо жоден профіль скрапінгу не спрацьовує або сайт змінив структуру, запишіть тексти вручну у файл `manual_poems.json` у корені репозиторію. Формат — масив об'єктів з полями `text`, `title`, `author`, `url` (URL можна лишити порожнім). Приклад уже доданий у файлі:

```json
[
  {
    "title": "Зразок вірша",
    "author": "Приклад Автор",
    "url": "",
    "text": "Це лише приклад рядків\nщоб показати формат файлу.\nЗамініть їх на свої вірші\nта додайте більше записів."
  }
]
```

Після заповнення передайте шлях до файлу у `--scraped` під час тренування або об'єднайте з даними, зібраними скрапером (обидва файли мають ідентичний формат).
